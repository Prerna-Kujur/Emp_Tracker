...
const EmployeeManagementApp = () => {
    ...
    const fetchEmployees = async (search = '', page = 1, limit = 5) => {
        ...
        try {
            ...
            if (data && data.employees && data.pagination) {
                setEmployeesData(data);
            } else {
                console.error('Invalid data structure:', data);
                notify('Failed to fetch employees. Invalid data structure.', 'error');
            }
        } catch (err) {
            console.error('Error fetching employees:', err);
            notify('Error fetching employees. Please try again later.', 'error');
        }
    }
       /*     setEmployeesData(data);
        } catch (err) {
